...
import au.com.shiftyjelly.pocketcasts.compose.ThemeLightColors
...
open class UserView @JvmOverloads constructor(
    ...
    open fun update(signInState: SignInState?) {
        updateProfileImageAndDaysRemaining()
...
class ExpandedUserView @JvmOverloads constructor(
    ...
    init {
        if (FeatureFlag.isEnabled(Feature.GRAVATAR_NATIVE_PICKER)) {
    ...
    private fun setGravatarAvatarPicker() {
        gravatarProfileCard.setContent {
            val colors = appTheme?.activeTheme?.let { themeTypeToColors(themeType = it) } ?: ThemeLightColors
            ...
            CompositionLocalProvider(
                LocalGravatarTheme provides object : GravatarTheme {
                    ...
                    override val colorScheme: ColorScheme
                        @Composable
                        get() = lightColorScheme(surface = colors.primaryUi02)
                },
            ) {
                GravatarTheme {
                    GravatarAvatarPicker()
                }
            }
        }
    }
    ...
    @Composable
    private fun GravatarAvatarPicker(modifier: Modifier = Modifier) {
        ...
        WordPressImagePicker(
            ...
            clientId = BuildConfig.GRAVATAR_CLIENT_ID,
            clientSecret = BuildConfig.GRAVATAR_CLIENT_SECRET,
            email = (signedInState as SignInState.SignedIn).email,
            listener = object : GravatarImagePickerWrapperListener {
                override fun onAvatarUploadStarted() {
                    Toast.makeText(context, "Uploading Avatar", Toast.LENGTH_LONG).show()
                }

                override fun onSuccess(response: Unit) {
                    Toast.makeText(
                        context,
                        "Avatar updated. Updates might take some time to sync with your Gravatar profile.",
                        Toast.LENGTH_LONG,
                    ).show()
                    updateProfileImageAndDaysRemaining()
                }
                ...
                override fun onError(errorType: ErrorType) {
                    Toast.makeText(context, "Avatar upload failed", Toast.LENGTH_LONG).show()
                }
            },
            modifier = modifier,
        )
