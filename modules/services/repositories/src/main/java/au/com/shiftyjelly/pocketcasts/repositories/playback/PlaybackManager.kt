...
//        get() = mediaSessionManager.mediaSession
    ...
    fun setup() {
        ...
        // TODO Could all of this be moved into init???
...
//        pocketCastsPlayer?.isStreaming ?: false
...
//    }
    ...
    suspend fun playNowSync(episode: Playable, forceStream: Boolean = false, playbackSource: AnalyticsSource = AnalyticsSource.UNKNOWN) {
        ...
        Timber.i("TEST123 PlaybackManager::playNowSync(), switchEpisode: $switchEpisode, isPlayerSwitchRequired: ${isPlayerSwitchRequired()}")
...
//            pocketCastsPlayer?.setPlayable(episode)
    ...
    fun castReconnected() {
        launch(Dispatchers.Main) {
            ...
            controller?.let {
                pocketCastsPlayer = playerManager.createCastPlayer(this@PlaybackManager::onPlayerEvent, it)
    ...
    private suspend fun onCompletion(episodeUUID: String?) {
        Timber.i("TEST123, PlaybackManager::onCompletion")
        Thread.dumpStack()
        if (resettingPlayer) {
            return
        }
        ...
        withContext(Dispatchers.Main) {
            ...
            controller?.let { controller ->
                if (castManager.isConnected()) {
                    pocketCastsPlayer = playerManager.createCastPlayer(this@PlaybackManager::onPlayerEvent, controller)
                    Timber.i("Creating media player of type CastPlayer.")
                } else {
                    pocketCastsPlayer = playerManager.createSimplePlayer(this@PlaybackManager::onPlayerEvent, controller)
                    Timber.i("Creating media player of type SimplePlayer.")
                }
            } ?: Timber.e("Controller is null, cannot create player.")
        }
